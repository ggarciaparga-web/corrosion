...
def ejecutar_simulacion_completa(tipo_ataque, inputs, ti):
    ...
    # Función local para calcular Mu (Momento Resistente)
    ...
    # Creamos el DataFrame de puntos críticos y CALCULAMOS MU AQUÍ
    df_points = pd.DataFrame(points).sort_values("Px (mm)").drop_duplicates("Px (mm)")
    df_points["Mu (kNm)"] = df_points.apply(lambda r: calc_mu_local(r["A1 (mm2)"], r["b"], r["d"]), axis=1)
    
    # --- 4. Generación de Matriz Final ---
    ...
    # Tiempo exacto de la recta vertical
